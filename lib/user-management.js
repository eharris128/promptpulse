...
import { showAccountCreationPrivacyNotice } from './privacy-notices.js';
...
export async function initializeDefaultUser() {
  try {
    // Additional safety check - this should already be handled by smartLogin
    const configPath = path.join(os.homedir(), '.promptpulse', 'config.json');
    if (fs.existsSync(configPath)) {
      try {
        const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        if (config.apiKey) {
          console.log('Warning: You already have a PromptPulse account configured.');
          console.log('This will create a new account and replace your current configuration.');
          console.log('');
          ...
          const shouldProceed = await new Promise((resolve) => {
            rl.question('Do you want to continue and create a new account? (y/N): ', (answer) => {
              rl.close();
          ...
          if (!shouldProceed) {
            console.log('Account creation cancelled. Your current configuration remains active.');
            return;
          }
          console.log('');
        }
      } catch (error) {
        // Config file exists but is corrupted, safe to proceed
      }
    }
    ...
    console.log('');
    showAccountCreationPrivacyNotice();
