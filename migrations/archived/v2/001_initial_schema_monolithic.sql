-- PromptPulse Initial Database Schema
-- Comprehensive schema with all features including teams, leaderboards, and display names
-- Created: 2025-06-28
-- This is a clean, consolidated schema for new deployments
...
CREATE TABLE user_email_preferences (
  ...
  -- Email settings
  email_reports_enabled BOOLEAN DEFAULT 0,    -- Whether to send reports
  report_frequency TEXT DEFAULT 'weekly',     -- 'daily', 'weekly', 'monthly'
  preferred_time TEXT DEFAULT '09:00',        -- HH:MM format
  timezone TEXT DEFAULT 'UTC',                -- User's timezone
  
  -- Last sent tracking (prevents duplicate sends)
...
-- Email send log for tracking delivery and debugging
CREATE TABLE email_send_log (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  email_type TEXT NOT NULL,               -- 'daily', 'weekly', 'monthly', 'test'
  email_address TEXT NOT NULL,            -- Email address used
  sent_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  status TEXT DEFAULT 'sent',             -- 'sent', 'failed', 'bounced'
  error_message TEXT,                     -- Error details if failed
  resend_email_id TEXT,                   -- Resend service email ID
...
CREATE TABLE teams (
  ...
  invite_code TEXT UNIQUE NOT NULL,       -- Unique invite code for joining
  is_active BOOLEAN DEFAULT 1,            -- Whether team is active
  max_members INTEGER DEFAULT 50,         -- Maximum number of members
...
-- Team invitations (for tracking pending invitations)
CREATE TABLE team_invitations (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  team_id INTEGER NOT NULL REFERENCES teams(id) ON DELETE CASCADE,
  email TEXT NOT NULL,                    -- Email address invited
  invited_by TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  invite_token TEXT UNIQUE NOT NULL,      -- Unique invitation token
...
-- Update users timestamp on changes
CREATE TRIGGER update_users_timestamp AFTER UPDATE ON users
BEGIN
  UPDATE users SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;

-- Update usage_data timestamp on changes
...
-- Update usage_sessions timestamp on changes
...
-- Update usage_blocks timestamp on changes
...
-- Update email preferences timestamp on changes
...
-- Update teams timestamp on changes
