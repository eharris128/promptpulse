...
async function checkUploadHistory(machineId, records, apiKey) {
  try {
  ...
  try {
    ...
    if (granularity === 'daily' || granularity === 'all') {
      ...
      if (dailyData.length > 0) {
        ...
        // Load session data to calculate session counts per day
        console.log('   Loading session data for session counts...');
        ...
        // Calculate session counts per date
        ...
        const dailyRecords = dailyData.map(day => ({
          ...
          sessions_count: sessionCountsByDate[day.date] || 0,
        ...
        const existingUploads = await checkUploadHistory(MACHINE_ID, historyCheck, API_KEY);
        ...
        if (newDailyRecords.length === 0) {
          ...
          await uploadData('/usage/daily/batch', { machineId: MACHINE_ID, data: newDailyRecords }, API_KEY);
    ...
    if (granularity === 'session' || granularity === 'all') {
      ...
      if (sessionData && sessionData.length > 0) {
        ...
        if (newSessionRecords.length === 0) {
          ...
          await uploadData('/usage/sessions/batch', { machineId: MACHINE_ID, data: newSessionRecords }, API_KEY);
    ...
    if (granularity === 'blocks' || granularity === 'all') {
      ...
      if (blockData && blockData.length > 0) {
        ...
        if (newBlockRecords.length === 0) {
          ...
          await uploadData('/usage/blocks/batch', { machineId: MACHINE_ID, data: newBlockRecords }, API_KEY);
