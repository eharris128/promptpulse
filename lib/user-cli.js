...
export async function userCommand(action, args) {
  try {
    ...
    switch (action) {
      case 'init':
        console.log('DEPRECATED: promptpulse user init');
        console.log('   Use: promptpulse login');
        console.log('');
        process.exit(1);
        break;
        
      case 'create':
        console.log('DEPRECATED: promptpulse user create');
        console.log('   Use: promptpulse login [username]');
        console.log('');
        process.exit(1);
        break;
        
      case 'login':
        console.log('DEPRECATED: promptpulse user login');
        console.log('   Use: promptpulse login <api-key>');
        console.log('   Note: Login now requires your API key for security');
        console.log('');
        process.exit(1);
        break;
        
      case 'list':
        console.log('DEPRECATED: promptpulse user list (admin-only feature)');
        console.log('   This command is no longer available for general users.');
        console.log('');
        process.exit(1);
        break;
      ...
      case 'config':
        ...
        switch (configAction) {
          case 'set':
            if (key === 'api-key') {
              console.log('DEPRECATED: promptpulse user config set api-key');
              console.log('   Use: promptpulse login <api-key>');
              console.log('');
              process.exit(1);
            }
          ...
          case 'get':
            if (key === 'api-key') {
              console.log('DEPRECATED: promptpulse user config get api-key');
              console.log('   Use: promptpulse whoami');
              console.log('');
              process.exit(1);
            }
            await getConfig(key);
          ...
          default:
            ...
            console.log('  promptpulse user config show                # Show current config (debugging)');
            console.log('');
            console.log('For authentication, use these commands instead:');
            console.log('  promptpulse login <api-key>                 # Login with API key');
            console.log('  promptpulse login                           # Create new account');
            console.log('  promptpulse whoami                          # Show current user');
        }
        break;
        
      case 'whoami':
        console.log('DEPRECATED: promptpulse user whoami');
        console.log('   Use: promptpulse whoami');
        console.log('');
        process.exit(1);
        break;
      ...
      default:
        console.log('PromptPulse authentication commands:');
        console.log('  promptpulse login                # Create new account (interactive)');
        console.log('  promptpulse login <api-key>      # Login with existing API key');
        console.log('  promptpulse logout               # Clear authentication');
        console.log('  promptpulse whoami               # Show current user');
        console.log('');
        console.log('Advanced/debugging commands:');
        console.log('  promptpulse user config show     # Show configuration file contents');
        break;
    }
  } catch (error) {
