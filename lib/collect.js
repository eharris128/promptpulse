...
import { getValidAccessToken } from "./auth-pkce.js";
...
async function getAuthHeaders() {
  ...
  // OAuth authentication only (Device Flow or PKCE Flow)
  if (config.auth_method === "oauth_device" || config.auth_method === "oauth_pkce") {
...
export async function collect(options = {}) {
  ...
  const isOAuth = userConfig.auth_method === "oauth_device" || userConfig.auth_method === "oauth_pkce";
  ...
  // Use auth0_id as user identifier for OAuth authentication
  const USER_ID = userConfig.auth0_id;
