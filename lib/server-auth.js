...
export async function createUserFromAuth0(auth0User) {
  ...
  try {
    ...
    const result = await db.sql`
      ...
      VALUES (${ksuid}, ${auth0User.email || null}, ${auth0User.nickname || auth0User.name}, ${auth0User.sub})
...
// Middleware for Express - supports API key authentication
// Note: Auth0 session authentication is handled by the Next.js client
export async function authenticateUser(req, res, next) {
  ...
  return res.status(401).json({ error: 'Authentication required. Please provide a valid X-API-Key header.' });
}
