...
export async function createUser(userData) {
  ...
  try {
    ...
    const result = await db.sql`
      INSERT INTO users (email, username, api_key_hash, ksuid)
      VALUES (${userData.email || null}, ${userData.username}, ${apiKeyHash}, ${ksuid})
      RETURNING id, email, username, ksuid, created_at
    `;
...
export async function getUserByApiKey(apiKey) {
  ...
  try {
    ...
    const allUsers = await db.sql`
      SELECT id, ksuid, email, username, created_at, api_key_hash
      FROM users
